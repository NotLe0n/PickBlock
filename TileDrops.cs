...
namespace PickBlock
{
    class TileDrops
    {
        public static void KillTile_GetItemDrops(int x, int y, Tile tileCache, out int dropItem, out int dropItemStack, out int secondaryItem, out int secondaryItemStack, bool includeLargeObjectDrops = false)
        {
            ...
            if (includeLargeObjectDrops)
            {
                switch (tileCache.type)
                {
                    case 21:
                    case 467:
                        dropItem = GetChestItemDrop(tileCache, tileCache.type);
                        break;
                    case 88:
                ...
                case 323:
                    {
                        ...
                        int j;
                        for (j = y; !Main.tile[x, j].IsActive || !Main.tileSolid[Main.tile[x, j].type]; j++)
                        {
                        }

                        if (Main.tile[x, j].IsActive)
                        {
        ...
        private static void KillTile_GetTreeDrops(int i, int j, Tile tileCache, ref bool bonusWood, ref int dropItem, ref int secondaryItem)
        {
            if (tileCache.frameX >= 22 && tileCache.frameY >= 198)
            {
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    {
                        int k;
                        for (k = j; Main.tile[i, k] != null && (!Main.tile[i, k].IsActive || !Main.tileSolid[Main.tile[i, k].type] || Main.tileSolidTop[Main.tile[i, k].type]); k++)
                        {
                        }
        ...
        private static void GetTreeBottom(int i, int j, out int x, out int y)
        {
            ...
            {
                while (y < Main.maxTilesY - 50 && (!tileSafely.IsActive || tileSafely.type == 323))
                {
                    y++;
            ...
            tileSafely = Framing.GetTileSafely(x, y);
            while (y < Main.maxTilesY - 50 && (!tileSafely.IsActive || TileID.Sets.IsATreeTrunk[tileSafely.type] || tileSafely.type == 72))
            {
                y++;
                tileSafely = Framing.GetTileSafely(x, y);
            }
        ...
        private static int GetChestItemDrop(Tile tile, int type)
        {
